# 職務経歴書

## 基本情報

|key|value|
|---|-----|
|名前|滝沢 郁紀|
|カナ|タキザワ フミキ|
|職業|バックエンドエンジニア|
|生年月日|1990年9月3日|
|MBTI診断|ISFP|

### 学歴
- 東京電機大学 情報環境学科	卒業　(2013年度)

### 要約
- 情報系大学卒業後、動画配信SIer、WEB広告代理店会社、SES会社で各クライアントで様々なプロジェクトに
エンジニアとして携わり、現在はフリーランスとして活動しています。WEBアプリケーション開発を約10年間
経験しており、要件定義からシステムテストまで一通りは経験しております。

### 活かせる経験・知識・スキル
- PHPによるWEBアプリケーション開発
- Ruby(Ruby on Rails)によるWEBアプリケーション開発
- GolangによるAPI開発
- gRPCを使った開発
- 要件定義、設計、開発、テスト、運用、保守までの上流工程から下流工程の一連の流れの経験
- 仮想環境、自動化ツールを使った開発


## スキル
### 言語
- Ruby2.3~3.0
- PHP5.4~8.0
- JavaScript
- TypeScript
- HTML
- Go

### 実務経験のあるフレームワーク
- Ruby on Rails4、5、8
- Laravel5、6、9
- CodeIgniter2
- Yii、Yii2
- ZendFramework1
- Symfony2.7
- CakePHP2
- Angular.js(ver1)
- Vue.js2,3
- Nuxt.js
- gin
- React

### 独学
- echo

### 実務経験があるミドルウェアサービスや使用ツール

- Linux
  - CentOS5〜7
  - Ubuntu
- AWS
  - EC2
  - ALB
  - DynamoDB
  - AWS Lambda 
  - S3
  - Amazon RDS
  - Amazon Elasti Cache
  - Cloud Watch
  - Simple Notification Service
  - Route53
  - Certificate Manager
  - Cloudfront
  - Amazon Personalize

- GCP
  - BigQuery
  - Cloud Storage
  - Google Cloud API
- Firebase
  - Cloud Messaging  
- Webサーバ
  - Apache2
  - Nginx
- DB
  - Mysql5.x、8.0
  - MariaDB
  - PostgreSQL
  - MongoDB
- 仮想環境、自動化ツール
  - Vagrant
  - Docker
  - Docker Compose
  - Chef
  - Ansible2
  - Selenium
  - CasperJS
  - capistrano
  - deployer(PHP)
- gRPC
  - gRPC gateway
- Cache  
  - memcached
  - Redis
  - Smarty
- CI
  - GitHub Actions 
  - CircleCI
  - Scrutinizer
  - codeclimate 
- デザインツール
  - Bootstrap
  - AdminLTE3
- エディタ
  - IntelliJ IDEA
  - Goland
  - WebStorm
  - PHPStorm
  - RubyMine
  - Sublime text
  - Atom
- その他
  - JIRA
  - Redmine
  - Waffle.io
  - Zenhub
 
## 強み
- 要件定義からシステムテストまで一通りは経験しております。
- 仮想環境、自動化ツールを使った開発環境の構築
- 未経験の技術でも自ら積極的に習得し、身に付けていく自信があります。

## 職務経歴

## 2018/11 - 現在 フリーランスエンジニア
- 職務: Webアプリケーションエンジニア
### ■参画した案件

### WEB動画配信によるオンライン学習サービス会社 (2020/04 - 現在)
- PHPを使ったサービスの改修
- LaravelやVue.jsを使った社外発信向けRSSの改修
- golang + React/Redux を使った社内管理システムの改修
- GoとNuxt.jsを使った高等教育機関向けDXプラットフォームの開発
- Go + gRPCを使ったアプリバックエンドの開発
- Ruby on Railsを使った人事管理サービスの開発
- Redisを使ったパフォーマンスチューニング
- DBやサーバーの監視、Cloud Watchのアラート設定、CI/CDの設定管理
- 14期上期おいて特別賞を受賞

### 受託開発会社 (2022/10 - 2023/3)
- Laravel9　+　Vue3　+　LINE APIを使ったアルバイト求人サービスの開発
- Gin(golang)のAPIを使った開発
- テックリード経験

### 某IaaS型クラウドサービス会社 (2018/10 - 2020/3)
- RailsAPIを使ったマイクロサービスの開発
- Rspecを使った自動テストの実装
- Laravelを使った課金システムの開発
- Docker-Composeを使った開発環境の導入
- GCPのAPIを使った開発
- Redisを使ったパフォーマンスチューニング

### 不動産物件の紹介サービス (2019/1 - 2019/6)
- Railsを使った物件検索サービスの開発

---


## 2016/1 - 2018/10 SES会社 正社員
- 職務: Webアプリケーションエンジニア

### ■参画した案件(すべてPHP案件)

### 某大手携帯会社 会員サービス案件 (2016/1 - 2016/6)
- キャッシュ機能を使った開発(PHPのAPC、memcached、Smarty)
- ドメイン駆動設計やスクラム開発
- ベテランエンジニアの方々と共に開発できたので、質の高い技術を吸収することができた。

### 勤怠管理システム (2016/7 - 2017/10)
- 特定顧客用の新規機能に伴い、お客様との要件定義からリリースまで携わりました。
- オンプレからAWSへの移行やレガシー環境をある程度モダンな環境にしました。
- PHPUnitの導入

### BtoBtoC写真ECサイト (2017/11 - 2018/6)
- 写真販売ECサイト及び管理画面等のシステム開発を担当
- 人手不足により4媒体において、フロントエンドとサーバサイドの両方を一人で任せられておりました。
- S3とCropperJSを使ったロゴアップロード機能の開発

### 物流プラットフォーム新規サービス(2018/7 - 2018/10)
- 新規サービスの開発
- APIの設計/開発
- PHPUnitの導入

---

## 2014/5 - 2015/12 広告代理店会社 正社員
- 職務: Webアプリケーションエンジニア
### 自社アフィリエイトサービスの開発(PHP、jquery)
- サービスのフロント画面と管理画面の開発　
- 営業、運用とのコミュニケーションを積極的にとり、常に報告・連絡・相談を意識して業務に取り組みました。
- セキュリティを意識したコード
- DB負荷の対策(インデックス設計、Viewテーブル作成、パーティションテーブル作成)

## 資格・認定
- Google Analytics認定 2016-10
- Google Adwords 基礎認定資格試験　2016-10
- Google Adwords 検索広告認定資格試験　2016-10
